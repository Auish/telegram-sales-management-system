 ...
 * @license MIT
 ...
 <?php 
  ...
  // include("./include/check.php");
    // استقبال الإيدي من عنوان الصفحة (URL)
    $id = isset($_GET['edit_id']) ? intval($_GET['edit_id']) : 0;

    // إذا تم الضغط على زر التعديل
    if(isset($_POST['btn-primary'])) {	
        // استخدام الإيدي من عنوان الصفحة
        if($id > 0) {
            $date_group = $conn->real_escape_string($_POST["grop_date"]);
            $month_start = $conn->real_escape_string($_POST["month_start"]);
            $month_end = $conn->real_escape_string($_POST["month_end"]);
            $coin = $conn->real_escape_string($_POST["grop_status"]);
            $price = $conn->real_escape_string($_POST["grop_pric"]);
            
            $sql = "UPDATE groups SET date_group = '$date_group', month_start = '$month_start', month_end = '$month_end', coin = '$coin', price = '$price' WHERE id = '$id'";
            $saveResult = $conn->query($sql);
            
            if($saveResult) {
                echo "<div class='alert alert-success'>تم تحديث البيانات بنجاح</div>";
                  // تحويل بعد ثانيتين
                header("refresh:1; url=list-catgro.php");
                exit;
            } else {
                echo "<div class='alert alert-danger'>حدث خطأ في التحديث: " . $conn->error . "</div>";
            }
        } else {
            echo "<div class='alert alert-danger'>خطأ في الإيدي</div>";
        }
    }

    // جلب البيانات الحالية للتعديل عليها
    $current_data = null;
    if($id > 0) {
...
<body class="navbar-fixed sidebar-nav fixed-nav">
    ...
    <div class="form-group">
        <label for="year" class="control-label col-lg-2">
            السنة
        </label>
        <div class="col-lg-7">
            <select class="form-control m-bot15" name="grop_date" id="year" required>
                <option value="<?php echo isset($current_data['date_group']) ? $current_data['date_group'] : ''; ?>"><?php echo isset($current_data['date_group']) ? $current_data['date_group'] : ''; ?></option>
                <?php
                $currentYear = date('Y');
    ...
    <div class="form-group">
        <label for="month" class="control-label col-lg-2">
            من شهر
        </label>
        <div class="col-lg-7">
            <select class="form-control m-bot15" name="month_start" id="month" required>
                <option value="<?php echo isset($current_data['month_start']) ? $current_data['month_start'] : ''; ?>"><?php echo isset($current_data['month_start']) ? $current_data['month_start'] : ''; ?></option>
    ...
    <div class="form-group">
        ...
        <div class="col-lg-7">
        <select class="form-control m-bot15" name="month_end" id="month" required>
                <option value="<?php echo isset($current_data['month_end']) ? $current_data['month_end'] : ''; ?>"><?php echo isset($current_data['month_end']) ? $current_data['month_end'] : ''; ?></option>
                <option value="1">1</option>
                <option value="2">2</option>
                <option value="3">3</option>
                <option value="4">4</option>
    ...
    <div class="form-group">
        <label for="CategoryStatus" class="control-label col-lg-2">
            العملة
        </label>
        <div class="col-lg-7">
            <select class="form-control m-bot15" name="grop_status" required>
                <option value="<?php echo isset($current_data['coin']) ? $current_data['coin'] : ''; ?>"><?php echo isset($current_data['coin']) ? $current_data['coin'] : ''; ?></option>
                <option value="USD">الدولار</option>
                <option value="الريال">الريال السعودي</option>
                <option value="الريال اليمني">الريال اليمني</option>
    ...
    <div class="form-group">
        ...
        <div class="col-lg-7">
            <input class="form-control" name="grop_pric" value="<?php echo isset($current_data['price']) ? $current_data['price'] : ''; ?>" type="number" step="0.01" min="0" required>
        </div>
    </div>

    <!-- حقل مخفي لحفظ التاريخ الكامل -->
    <input type="hidden" name="full_date" id="full_date">

    <div class="form-actions">
        <button type="submit" class="btn btn-primary" name="btn-primary">حفظ</button>
    ...
    <script>
        ...
        document.getElementById('roleFilter').addEventListener('change', function() {
            ...
            let rows = document.querySelectorAll('#dataTable tbody tr');
            rows.forEach(row => {
                let roleCell = row.querySelectorAll('td')[3];
                if (filterValue === '' || roleCell.textContent === filterValue) {
                    row.style.display = '';
                } else {
                    row.style.display = 'none';
                }
        ...
        // JavaScript for displaying selected row data
        let checkboxes = document.querySelectorAll('.rowCheckbox');
        checkboxes.forEach(checkbox => {
        ...
        function displaySelectedData() {
            ...
            rows.forEach(row => {
                ...
                if (checkbox.checked) {
                    ...
                    let rowData = {
                        ...
                        dateRegistered: cells[2].textContent,
            ...
            let displayDiv = document.getElementById('selectedRowData');
            if (selectedData.length > 0) {
                displayDiv.innerHTML = '<h5>Selected Row Data:</h5>';
                selectedData.forEach(data => {
                    displayDiv.innerHTML += `<p>Username: ${data.username}, Date Registered: ${data.dateRegistered}, Role: ${data.role}, Status: ${data.status}</p>`;
                });
            } else {
