...
// الحصول على الإيدي من عنوان الرابط
$user_id = isset($_GET['edit_id']) ? intval($_GET['edit_id']) : 0;

if ($user_id > 0) {
    // استعلام لمستخدم واحد فقط
    $query = "SELECT 
                u.*,
                COUNT(CASE WHEN o.status = 'مكتمل' THEN 1 END) AS completed_orders,
                COUNT(CASE WHEN o.status = 'مقبول' THEN 1 END) AS accepted_orders,
                COUNT(CASE WHEN o.status = 'مرفوظ' THEN 1 END) as quoted_orders,
                COALESCE(w.balance, 0) AS wallet_balance
              FROM user u
              LEFT JOIN orders o ON u.id = o.id_user
              LEFT JOIN wallets w ON u.id = w.id_user
              WHERE u.id = ?
    ...
    // إذا كان هناك خطأ، دعنا نرى التفاصيل
    if (!$result) {
    ...
    <table class="table table-bordered table-striped table-condensed">
        <thead>
            <tr>
                ...
                <th>الطلبات المرفوظه</th>
                <th>الطلبات المكتمل</th>
                <th>رصيد المحفظة</th>
        ...
        <tbody>
            ...
            if ($result && $result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $status_class = ($status == 'نشط') ? 'tag-warning' : 'tag-danger';
                    
                    // تحديد النص حسب الجنس
                    $gender = $row['gender'] ?? 'ذكر';
                    $gender_text = ($gender == 'female') ? 'أنثى' : 'ذكر';
                    
                    // تنسيق الصورة
                    $image_path = !empty($row['image']) ? $row['image'] : 'img/avatars/6.jpg';
            ?>
            <tr>
                <td><?php echo $counter; ?></td>
                <td><?php echo htmlspecialchars($row['username'] ?? ''); ?></td> 
                <td><?php echo htmlspecialchars($row['status'] ?? 'مستخدم'); ?></td>  
                <td><?php echo $row['accepted_orders'] ?? 0; ?></td>
                <td><?php echo $row['quoted_orders'] ?? 0; ?></td>
                <td><?php echo $row['completed_orders'] ?? 0; ?></td>
                <td><?php echo number_format($row['wallet_balance'] ?? 0, 2); ?> ر.س</td>
...
function deleteUser(userId) {
    if (confirm('هل أنت متأكد من حذف هذا المستخدم؟')) {
        // إرسال طلب الحذف عبر AJAX أو redirect
