    ...
    if($order_id > 0 && !empty($status)) {
        ...
        if(in_array($status, $valid_statuses)) {
            ...
            if($stmt->execute()) {
                ...
                echo "<script>
                setTimeout(function(){
                    window.location.href = 'list-ordar.php';
    ...
    <tbody>
        ...
        if($result && $result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                echo "<td><a href='" . htmlspecialchars($row['group_url']) . "' target='_blank'>" . htmlspecialchars($row['group_url']) . "</a></td>";
                ...
                echo "<td>
                        <form method='post' class='update-form' onsubmit='return updateOrderStatus(this)'>
                            ...
                            <select name='status' onchange='this.form.submit()' style='background-color: " . getStatusColor($row['status']) . ";'>
                                ...
                                <option value='نقل الملكية' " . ($row['status'] == 'نقل الملكية' ? 'selected' : '') . ">نقل الملكية</option>
            ...
            echo "<tr><td colspan='5' style='text-align: center;'>لا توجد طلبات</td></tr>";
        }
        ?>
    </tbody>
</table>
